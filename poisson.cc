...
void read1D(std::vector<float> &f, char *fname, char *dsetname, hsize_t Nloc,
  ...
  if (ndims == 4)
    if (!rank)
      printf("Warning: Reading 1D field from 3d dataset!\n");
...
void write1D(std::vector<float> &f, char *fname, char *dsetname, hsize_t Nloc,
  ...
  const hsize_t chunk[3] = {256, 256, 1};
  H5Pset_chunk(cparms, 3, chunk);
  ...
  dataset = H5Dcreate(file, dsetname, H5T_NATIVE_FLOAT, dataspace, H5P_DEFAULT,
                      cparms, H5P_DEFAULT);
...
void write3D(std::vector<float> &f, char *fname, char *dsetname, hsize_t Nloc,
  ...
  if (f.size() / (Nloc * M * M) != 3)
    if (!rank)
      printf("Warning: vector to write is not 3D!\n");
...
float residual(std::vector<float> &f, std::vector<float> &phi, hsize_t Nloc,
  ...
  for (i = 0; i < (long long)Nloc; i++) {
    ...
    for (j = 0; j < M; j++)
      for (k = 0; k < M; k++) {
        nabla2f = phi[loc(ii + 1, j, k, M)] + phi[loc(ii - 1, j, k, M)] +
                  phi[loc(ii, j + 1, k, M)] + phi[loc(ii, j - 1, k, M)] +
                  phi[loc(ii, j, k + 1, M)] + phi[loc(ii, j, k - 1, M)] -
                  6 * phi[loc(ii, j, k, M)];
        nabla2f /= h * h;
        res += (nabla2f - f[loc(i, j, k, M)]) * (nabla2f - f[loc(i, j, k, M)]);
      }
  }
